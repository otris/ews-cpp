...
#pragma warning(suppress : 6262)
    ...
    TEST(PropertyPathTest, ConstructFromURI)
    {
        ...
        EXPECT_STREQ("<t:FieldURI FieldURI=\"folder:FolderId\"/>", path.to_xml().c_str());

        path = "item:DisplayCc";
        EXPECT_STREQ("<t:FieldURI FieldURI=\"item:DisplayCc\"/>", path.to_xml().c_str());

        path = "message:ToRecipients";
        EXPECT_STREQ("<t:FieldURI FieldURI=\"message:ToRecipients\"/>", path.to_xml().c_str());

        path = "meeting:IsOutOfDate";
        EXPECT_STREQ("<t:FieldURI FieldURI=\"meeting:IsOutOfDate\"/>", path.to_xml().c_str());

        path = "meetingRequest:MeetingRequestType";
        EXPECT_STREQ("<t:FieldURI FieldURI=\"meetingRequest:MeetingRequestType\"/>", path.to_xml().c_str());

        path = "calendar:Start";
        EXPECT_STREQ("<t:FieldURI FieldURI=\"calendar:Start\"/>", path.to_xml().c_str());

        path = "task:AssignedTime";
        EXPECT_STREQ("<t:FieldURI FieldURI=\"task:AssignedTime\"/>", path.to_xml().c_str());

        path = "contacts:Children";
        EXPECT_STREQ("<t:FieldURI FieldURI=\"contacts:Children\"/>", path.to_xml().c_str());

        path = "distributionlist:Members";
        EXPECT_STREQ("<t:FieldURI FieldURI=\"distributionlist:Members\"/>", path.to_xml().c_str());

        path = "postitem:PostedTime";
        EXPECT_STREQ("<t:FieldURI FieldURI=\"postitem:PostedTime\"/>", path.to_xml().c_str());
    ...
    TEST(IndexedPropertyPath, ToXML)
    {
        ...
        EXPECT_STREQ("<t:IndexedFieldURI FieldURI=\"contacts:PhoneNumber\" FieldIndex=\"BusinessPhone\"/>",
                        path.to_xml().c_str());
    }
